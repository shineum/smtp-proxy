...
import { fetchUsers, createUser, deleteUser, updateUserStatus } from '../api/resources';
...
export default function UserListPage() {
  ...
  const [form, setForm] = useState({ email: '', password: '', account_type: 'user', username: '', role: 'member' });
  ...
  const createMutation = useMutation({
    mutationFn: () => createUser({
      ...form,
      group_id: me?.current_group.group_id,
    }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['users'] });
      setIsCreateOpen(false);
      setForm({ email: '', password: '', account_type: 'user', username: '', role: 'member' });
    },
  });
  ...
  return (
    <PageSection>
      ...
      <Modal
        ...
        actions={[
          <Button key="create" onClick={() => createMutation.mutate()} isDisabled={!form.email || createMutation.isPending}>
      ...
      >
        <Form>
          ...
          <FormGroup label="Account Type" fieldId="user-type">
            <FormSelect id="user-type" value={form.account_type} onChange={(_e, v) => setForm({ ...form, account_type: v })}>
